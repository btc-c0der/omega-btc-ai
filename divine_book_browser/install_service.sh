#!/bin/bash
# Installation script for Divine Book Browser Server

# Determine the script directory
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
FULL_PATH=$(realpath "$SCRIPT_DIR")
PLIST_PATH="$HOME/Library/LaunchAgents/com.omega.divinebrowser.plist"

echo "✨ Installing Divine Book Browser Service ✨"
...
echo "Divine Book Browser is now available at: http://localhost:8888"
...
echo "  - Check status:    launchctl list | grep com.omega.divinebrowser"
echo "  - View logs:       $SCRIPT_DIR/divine_server.log"
echo "  - Run manually:    $SCRIPT_DIR/run_server.sh"
echo
echo "You can access the Divine Book Browser at: http://localhost:8888 (or next available port)" 