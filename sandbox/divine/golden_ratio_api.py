...
def index():
    ...
    return send_file('sandbox/divine/divine_alignment_dashboard.html')
...
def serve_latest_chart():
    ...
    png_files = [f for f in os.listdir('.') if 'btc_golden_ratio_7yr_' in f and f.endswith('.png')]
...
def generate_trading_data(current_price: Optional[float] = None) -> Dict[str, Any]:
    ...
    # Calculate percentage difference
    diff_percent = ((current_price - golden_price) / golden_price) * 100 if golden_price else 0
...
def serve_css(filename):
    """Serve CSS files."""
    return send_file(f'sandbox/divine/assets/css/{filename}')
...
if __name__ == '__main__':
    # Run the Flask app on port 5051
