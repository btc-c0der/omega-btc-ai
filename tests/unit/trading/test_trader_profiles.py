...
from omega_ai.trading.profiled_futures_trader import ProfiledFuturesTrader, Position
...
def mock_redis():
    ...
    with patch('redis.StrictRedis') as mock_redis:
        ...
        mock_client._list_data = {}
        mock_client._hash_data = {}
...
def test_trader_take_profit_levels(profile_type):
    ...
    # Newbies often use unrealistic targets or exit too early
        ...
        if len(long_take_profits) == 1:
            ...
            # Multiple take profit levels with conservative targets
            total_percentage = sum(tp["percentage"] for tp in long_take_profits)
            assert abs(total_percentage - 1.0) < 0.01  # Should sum to 100%
            
            # First target should be conservative (around 2x risk)
            first_tp = long_take_profits[0]["price"]
            risk_distance = abs(TestConfig.TEST_PRICE - first_tp)
            assert 1.5 <= risk_distance / TestConfig.TEST_PRICE <= 3.0  # 1.5x to 3x risk
            
            # Second target should be wider (around 2.5x risk)
            if len(long_take_profits) > 1:
                second_tp = long_take_profits[1]["price"]
                risk_distance = abs(TestConfig.TEST_PRICE - second_tp)
                assert 2.0 <= risk_distance / TestConfig.TEST_PRICE <= 4.0  # 2x to 4x risk
...
def test_trader_position_management(profile_type):
    ...
    stop_loss = trader.positions[0].stop_loss
    if stop_loss is not None:
        trader.update_price(float(stop_loss))  # Ensure stop_loss is float
    trader.manage_open_positions()
...
def test_trader_psychological_changes(profile_type):
    ...
    position = Position(
        ...
        size=1000.0,
        leverage=int(2.0),  # Convert float to int
        entry_time=datetime.now(),
        stop_loss=TestConfig.TEST_PRICE * 0.90,
        take_profits=[{"percentage": 1.0, "price": TestConfig.TEST_PRICE * 1.05}],
        entry_reason="Test psychological changes"
    )
    ...
    trader.update_price(TestConfig.TEST_PRICE)  # Use update_price instead of direct assignment
    position = Position(
        id="test_position_2",  # Add missing id
        direction="LONG",
        ...
        entry_reason="Test losing trade"  # Add missing entry_reason
    )
    trader.positions.append(position)
    ...
    for i in range(4):
        position = Position(
            id=f"test_position_{i+3}",  # Add unique id
            direction="LONG",
            entry_price=TestConfig.TEST_PRICE * 1.05,
            size=1000.0,
            leverage=int(2.0),  # Convert float to int
            entry_time=datetime.now(),
            stop_loss=TestConfig.TEST_PRICE * 0.95,
            entry_reason=f"Test consecutive loss #{i+1}"  # Add missing entry_reason
        )
        trader.positions.append(position)
...
def test_trader_redis_integration(mock_redis):
    ...
    # Initialize mock Redis data structures
    mock_redis._list_data = {}
    mock_redis._hash_data = {}
    ...
    with patch('redis.StrictRedis', return_value=mock_redis):
        ...
        positions_key = f"trader:positions:strategic"
        trades_key = f"trader:trades:strategic"
        metrics_key = f"trader:metrics:strategic"
        
        assert positions_key in mock_redis._list_data
        assert len(mock_redis._list_data[positions_key]) == 1
        assert trades_key in mock_redis._list_data
        assert len(mock_redis._list_data[trades_key]) == 1
        assert metrics_key in mock_redis._hash_data
        ...
        # Verify trade result was stored
        assert len(mock_redis._list_data[trades_key]) == 2
        assert metrics_key in mock_redis._hash_data
        
        # Verify trader metrics were stored
        metrics = mock_redis._hash_data[metrics_key]
        ...
        print(f"   Stored positions: {len(mock_redis._list_data[positions_key])}")
        print(f"   Stored trades: {len(mock_redis._list_data[trades_key])}")
        print(f"   Stored metrics: {list(mock_redis._hash_data[metrics_key].keys())}")
...
def test_trader_edge_cases(profile_type):
    ...
    trader.open_position("LONG", "Test position size limit", int(trader.max_leverage))
    assert len(trader.positions) == 1
    assert trader.positions[0].size <= max_position_size
    
    # Test multiple positions
    for _ in range(3):
        trader.open_position("LONG", "Test multiple positions", 2)
...
def test_trader_performance_metrics(mock_redis_conn):
    ...
    trader.open_position("LONG", "Test trade", leverage=int(1.0))  # Convert float to int
    ...
    for pos in trader.positions:
        trader._close_position(pos, "Test close")  # Use internal method to handle Position
    
    # Open a short position
    update_price_safely(trader, mock_redis_conn, new_price)
    trader.open_position("SHORT", "Test trade", leverage=int(1.0))  # Convert float to int
...
def test_trader_error_handling(mock_redis_conn):
    ...
    # Test invalid position direction
    with pytest.raises(ValueError, match="Invalid position direction"):
        trader.open_position("INVALID", "Test invalid direction", 2)
    
    # Test invalid leverage
    with pytest.raises(ValueError, match="Invalid leverage value"):
        trader.open_position("LONG", "Test invalid leverage", trader.max_leverage + 1)
    
    # Test invalid position size
    with pytest.raises(ValueError, match="Position size exceeds maximum allowed"):
        trader.open_position("LONG", "Test invalid size", leverage=int(trader.capital * (trader.max_leverage + 1)))  # Convert float to int
    ...
    with pytest.raises(ValueError, match="Price must be positive"):
        trader.update_price(0.0)  # Use 0.0 instead of None
        
    # Test price update with invalid type
    with pytest.raises(ValueError, match="Price must be positive"):
        trader.update_price(0.0)  # Use 0.0 instead of string
