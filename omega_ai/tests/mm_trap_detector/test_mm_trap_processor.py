...
@pytest.fixture
def setup_price_environment(mock_redis):
    """Divine price environment setup for tests."""
    def _setup(current_price, prev_price=None):
        mock_redis.set("last_btc_price", str(current_price))
        mock_redis.set("last_btc_volume", "10000")
        
        if prev_price:
            mock_redis.set("prev_btc_price", str(prev_price))
...
class TestMMTrapDetector:
    ...
    def test_liquidity_grab_detection(self, mock_sleep, mock_send_alert, mock_register_trap, 
        ...
        self._run_detector_once(mock_sleep)
    ...
    def test_half_liquidity_grab_detection(self, mock_sleep, mock_send_alert, mock_register_trap, 
        ...
        current_price = 60000.0
        prev_price = 59400.0  # $600 change, between half and full threshold
        price_change_pct = (current_price - prev_price) / prev_price
        abs_change = abs(current_price - prev_price)
    ...
    def test_initial_price_handling(self, mock_sleep, mock_send_alert, mock_register_trap, 
        ...
        setup_price_environment(current_price)  # No prev_price
        ...
        mock_insert_subtle.assert_called()
        subtle_call = mock_insert_subtle.call_args[0]
        assert subtle_call[2] is not None, "Should create a valid previous price"
        assert subtle_call[2] != current_price, "Previous price should differ from current"
    ...
    def test_potential_fake_pump(self, mock_sleep, mock_send_alert, mock_register_trap, 
        ...
        # Setup test data - price increase above PRICE_PUMP_THRESHOLD (0.02 = 2# filepath: /Users/fsiqueira/Desktop/GitHub/omega-btc-ai/omega_ai/tests/mm_trap_detector/test_mm_trap_processor.py
