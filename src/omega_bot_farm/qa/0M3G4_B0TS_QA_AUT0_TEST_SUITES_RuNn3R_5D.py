...
class QuantumTestService:
    ...
    def __init__(self, project_root: str, config: Optional[Dict[str, Any]] = None):
        ...
        # Git manager for tracking uncommitted files
        self.git_manager = GitManager(self.project_root)
        
        # New: K8s Matrix Surveillance
        ...
        self.k8s_matrix_mode = self.config.get('k8s_matrix_mode', False)
        
        # Track last scan times
        self.last_uncommitted_scan = 0
        self.last_k8s_matrix_report = 0
        self.uncommitted_scan_interval = self.config.get('uncommitted_scan_interval', 300)  # 5 minutes default
        self.k8s_report_interval = self.config.get('k8s_report_interval', 600)  # 10 minutes default
    ...
    def start(self) -> None:
        ...
        # Start K8s surveillance if configured
        if self.k8s_matrix_mode or self.full_omega_mode:
            self.k8s_surveillance.start()
        ...
        # If in full Omega mode, start with initial scans
        if self.full_omega_mode:
            ...
            # Initial K8s Matrix report if available
            if self.k8s_matrix_mode and self.k8s_surveillance.available:
    ...
    def stop(self) -> None:
        ...
        # Stop K8s surveillance
        if self.k8s_surveillance:
            self.k8s_surveillance.stop()
    ...
    def _check_periodic_tasks(self) -> None:
        ...
        # Check if K8s matrix report should be generated
        if (self.k8s_matrix_mode or self.full_omega_mode) and \
           now - self.last_k8s_matrix_report >= self.config['k8s_report_interval'] and \
           self.k8s_surveillance.available:
            self.k8s_surveillance.print_matrix_report()
            self.last_k8s_matrix_report = now
...
# Kubernetes surveillance for Matrix integration
